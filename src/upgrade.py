...
class LevelUp:
    def __init__(self, screen, player):
        ...
        spacing = 50
        card_size = (150, 220)
        total_width = len(self.base_images) * card_size[0] + (len(self.base_images) - 1) * spacing
        start_x = (screen_width - total_width) // 2

        for i, (key, img) in enumerate(self.base_images.items()):
            scaled_img = pygame.transform.scale(img, card_size)
            rect = scaled_img.get_rect(topleft=(start_x + i * (card_size[0] + spacing), screen_height // 2 - 100))
            self.card_data[key] = {
                'image': scaled_img,
                'rect': rect,
                'hovered': False
        ...
        if self.visible:
            ...
            for name, data in self.card_data.items():
                ...
                if rect.collidepoint(mouse_pos):
                    data['hovered'] = True
                    enlarged = pygame.transform.scale(data['image'], (165, 240))
    ...
    def apply_upgrade(self, name):
        ...
        print(f"New speed: {self.player.base_speed + self.player.speed_bonus}")
